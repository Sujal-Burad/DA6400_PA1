...
# num_steps not required as this minigrid environment has terminated or truncated information
# parser.add_argument('--num_steps', type=int, default=800, help='The number of steps.') # The number can be changed according to need
...
for seed in args.seed:
    ...
    if args.algorithm == "q_learning":
        ...
        wandb.init(project="minigrid_rl_experiment_q_learning_minimizing_regret", name=file_name + "_seed_" + str(seed))

        # define the q-table
        q_table = Qtable(env.observation_space, env.action_space)

        # instantiate the q-learner agent
        q_learner = QLearner(args.alpha, args.gamma, args.tau, q_table, env, seed, args.tau_decay)
        # learn the q-table
        reward_list, total_steps_list = q_learner.learn(args.num_episodes)

        for reward in reward_list:
            wandb.log({"Episode_return": reward})
        for step in total_steps_list:
            wandb.log({'Number of steps in episode': step})
        
        wandb.log({'Max episode return': max(reward_list)})
        wandb.log({'Min episode return': min(reward_list)})
        wandb.log({'Max episode steps': max(total_steps_list)})
        wandb.log({'Min episode steps': min(total_steps_list)})
        wandb.finish()
        ...
        wandb.init(project="minigrid_rl_experiment_sarsa_minimizing_regret", name=file_name + "_seed_" + str(seed))
        # define the q-table
        q_table = Qtable(env.observation_space, env.action_space)

        # instantiate the q-learner agent
        sarsa_learner = SarasLearner(args.alpha, args.gamma, args.epsilon, q_table, env, seed, args.epsilon_decay)
        # learn the q-table
        reward_list, total_steps_list = sarsa_learner.learn(args.num_episodes)
    ...
    total_steps_list_seed.append(total_steps_list)
...
plot_reward(reward_list_seed=total_steps_list_seed,
            num_episodes=args.num_episodes,
            file_name='total_steps' + file_name + '.png')
